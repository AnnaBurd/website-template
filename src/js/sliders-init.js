...
console.log("Hello from slider-init");

let sliderPrice = document.getElementById("priceSlider");
let sliderArea = document.getElementById("areaSlider");

model.loadLandplotsData();
let maxPrice = model.getMaxPrice();
let maxArea = model.getMaxArea();

maxPrice = Math.ceil(maxPrice / 10) * 10;
maxArea = Math.ceil(maxArea / 100) * 100;

console.log(maxPrice, maxArea);

let JSSLIDERPRICE = noUiSlider.create(sliderPrice, {
  start: [0, maxPrice],
  tooltips: [true, true],
  connect: true,
  format: {
    ...
    from: function (value) {
      // Receives a string, should return a number - 'from' the formatted value
...
let JSSLIDERAREA = noUiSlider.create(sliderArea, {
  start: [0, maxArea],
  tooltips: [true, true],
  connect: true,
  format: {
...
console.log(JSSLIDERPRICE, JSSLIDERAREA);

export { JSSLIDERAREA, JSSLIDERPRICE };
